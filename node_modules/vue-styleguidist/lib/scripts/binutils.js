...
var logger = (0, _glogg["default"])('vsg');
...
var getProgressPlugin = function getProgressPlugin(msg) {
  var bar = new _cliProgress.Bar({
    format: "".concat(msg, " ").concat(_kleur["default"].green('{bar}'), " {percentage}%")
  }, _cliProgress.Presets.rect);
  return {
    plugin: new _webpack.ProgressPlugin(function (percentage) {
...
function commandBuild(config) {
  ...
  if (config.progressBar !== false && !(config.webpackConfig.plugins || []).some(function (p) {
    return p.constructor === _webpack.ProgressPlugin;
  })) {
    ...
    bar = localBar;
    config.webpackConfig.plugins = [].concat(_toConsumableArray(config.webpackConfig.plugins || []), [plugin]);
    bar.start(1, 0);
  }
...
function commandHelp() {
  console.log([_kleur["default"].underline('Usage'), '', '    ' + _kleur["default"].bold('styleguidist') + ' ' + _kleur["default"].cyan('<command>') + ' ' + _kleur["default"].yellow('[<options>]'), '', _kleur["default"].underline('Commands'), '', '    ' + _kleur["default"].cyan('build') + '           Build style guide', '    ' + _kleur["default"].cyan('server') + '          Run development server', '    ' + _kleur["default"].cyan('help') + '            Display React Styleguidist help', '', _kleur["default"].underline('Options'), '', '    ' + _kleur["default"].yellow('--config') + '        Config file path', '    ' + _kleur["default"].yellow('--open') + '          Open Styleguidist in the default browser', '    ' + _kleur["default"].yellow('--verbose') + '       Print debug information'].join('\n'));
}
/**
 * @param {object} urls
...
function printServerInstructions(urls, publicPath) {
  ...
  console.log();
  console.log("  ".concat(_kleur["default"].bold('Local:'), "            ").concat(urls.localUrlForTerminal + publicPath));

  if (urls.lanUrlForTerminal) {
    console.log("  ".concat(_kleur["default"].bold('On your network:'), "  ").concat(urls.lanUrlForTerminal + publicPath));
  }
...
function printBuildInstructions(config) {
  console.log('Style guide published to:\n' + _kleur["default"].underline(config.styleguideDir || ''));
}
/**
...
function printErrorWithLink(message, linkTitle, linkUrl) {
  console.error("".concat(_kleur["default"].bold.red(message), "\n\n").concat(linkTitle, "\n").concat(_kleur["default"].underline(linkUrl), "\n"));
}
/**
 * @param {string} header
 * @param {object} errors
...
function printStatus(text, type) {
  if (type === 'success') {
    console.log(_kleur["default"].inverse.bold.green(' DONE ') + ' ' + text);
  } else if (type === 'error') {
    console.error(_kleur["default"].inverse.bold.red(' FAIL ') + ' ' + _kleur["default"].red(text));
  } else {
    console.error(_kleur["default"].inverse.bold.yellow(' WARN ') + ' ' + _kleur["default"].yellow(text));
  }
}
/**
...
function verbose(header, object) {
  logger.debug(_kleur["default"].bold(header) + '\n\n' + (0, _qI.stringify)(object));
}